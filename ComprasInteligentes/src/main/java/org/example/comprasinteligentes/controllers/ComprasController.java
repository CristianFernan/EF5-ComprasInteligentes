...
public class ComprasController{ //00016623 Clase controladora para manejar la lógica de las compras
    ...
    public void initialize() { //00016623 Función de inicialización de la vista que se ejecuta antes de lo demás para pre configuraciones necesarias.
        monto.setCellValueFactory(new PropertyValueFactory<>("monto")); //00016623 asignarle una value factory a la columna para que utilice el atributo monto para guardarlos
        descripcion.setCellValueFactory(new PropertyValueFactory<>("descripcion")); //00016623 asignarle una value factory a la columna para que utilice el atributo descripcion para guardarlos
        fecha.setCellValueFactory(new PropertyValueFactory<>("fecha")); //00016623 asignarle una value factory a la columna para que utilice el atributo fecha para guardarlos
        tarjeta.setCellValueFactory(new PropertyValueFactory<>("tarjeta")); //00016623 asignarle una value factory a la columna para que utilice el atributo tarjeta para guardarlos
        cliente.setCellValueFactory(new PropertyValueFactory<>("cliente")); //00016623 asignarle una value factory a la columna para que utilice el atributo cliente para guardarlos
    ...
    private void cargarDatosCompra() { //00016623 Función para cargar los datos de las compras en la tabla
        ...
        try { //00016623 Inicio del bloque try para manejar excepciones SQL
            ...
            ResultSet rs = st.executeQuery("select montoTotal as monto, descripcion, fechaCompra as fecha, concat(t.tipo, ', ', f.facilitador) as tarjeta, concat (cl.nombre, ' ', cl.apellido) as cliente from tbCompra c inner join tbtarjeta t on c.idTarjeta = t.id inner join tbfacilitador f on t.idFacilitador = f.id inner join tbcliente cl on t.idCliente = cl.id"); // 00016623 Ejecuta una consulta SQL para obtener los datos de las compras
